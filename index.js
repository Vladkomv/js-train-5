...
function multiplyArray(array) {
  let prod = array[0]; // Ініціалізуємо змінну для зберігання добутку
  for (const item of array) {
    prod *= item;
  } // Проходимося по всіх значеннях масиву
  // Множимо кожен елемент на загальний добуток
  return prod; // Повертаємо добуток
}
...
function modifyArray(array) {
  ...
  return console.log(array.toString()); // Виводимо масив перетворений в рядок
}
...
// Завдання 6: Функція приймає деструктуризований масив чисел де будуть перший елемент та масив всіх інших.
...
function printStrings(array) {
  return console.log(array.toString().trim().repeat(2)); // Перетворюємо масив в рядок і видаляемо пробіли та виводимо його на консоль
}
...
function concatenateStrings(array, subString) {
  ...
  return console.log(array.toString().includes(subString)); //Перевіряємо чи містить рядок підрядок
  // Виводимо результат
}
...
function addTenToEach(array) {
  let sumArr = []; // Ініціалізуємо масив через конструктор з такою же кількістю елементів як в вхідному масиві для зберігання результату
  for (const index in array) {
    sumArr[index] = array[index] + 10;
  } // Перебираємо масив за допомогою циклу for-in
  // Додаємо до поточного елемента 10 та додаємо результат в новий масив
  return console.log(sumArr); // Виводимо результат
}
...
function getEvenNumbers(arr) {
  let arrnew = []; // Створюємо пустий масив для зберігання результату
  for (const index in arr) {
    if (arr[index] % 2 === 0) {
      arrnew = [...arrnew, arr[index]];
    }
...
function multiplyByIndex(arr) {
  let arrnew = []; // Створюємо пустий масив для зберігання результату
  for (const index in arr) {
    let n = arr[index] * index;
    arrnew = [...arrnew, n];
  } // Проходимо через кожен елемент вхідного масиву
  // Помножуємо число на його індекс і додаємо до масиву результату
  return arrnew; // Повертаємо масив результату
}
...
function replaceNumbers(arr) {
  ...
  for (const item of arr) {
    if (item > 10) {
      arrnew = [...arrnew, "Greater than 10"];
    } else if (item <= 10) {
      arrnew = [...arrnew, "Less than or equal to 10"];
    }
  } // Перевіряємо, чи число більше за 10
  return arrnew; // Якщо число більше за 10, додаємо рядок "Greater than 10" до масиву результату
